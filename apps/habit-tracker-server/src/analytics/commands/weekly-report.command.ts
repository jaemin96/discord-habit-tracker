...
@Injectable()
export class WeeklyReportCommand implements ICommand {
  name = '주간리포트';
  description = '이번 주 체크인 통계 리포트';
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const stats = await this.analyticsService.getWeeklyStats(interaction.user.id);

    if (stats.total === 0) {
      await interaction.editReply('이번 주 체크인 기록이 없습니다.');
      return;
    }

    const embed = buildStatsEmbed(stats, '📊 주간 리포트');
